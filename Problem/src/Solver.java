...
public class Solver {
    ...
    public void solve() throws FileNotFoundException {
        ...
        for (int num_el = 0; num_el < gd.ne; num_el++) {
            ...
            for (int i = 0; i < 4; i++) {
                alfa = grid.elements[num_el].alfa;
                specificHeat = grid.elements[num_el].specificHeat;
                conductivity = grid.elements[num_el].conductivity;
                density = grid.elements[num_el].density;
...
//                }
                ...
                for (int i = 0; i < 4; i++) {
                    for (int j = 0; j < 4; j++) {
                        C_local = specificHeat * density * el.dN[punktycal][i] * el.dN[punktycal][j] * detj;
                        H_local[i][j] += conductivity * (dNdx[i] * dNdx[j] + dNdy[i] * dNdy[j]) * detj + C_local / gd.simulationStepTime;// obliczanie macierzy H w oparcu o wczesniejsze 2 macierze
...
//                }
            ...
            for (int ipow = 0; ipow < grid.elements[num_el].liczbaPowierzchni; ipow++) {    // poruszamy sie po powierzchniach kontaktowych z otoczeniem
                ...
                for (int p = 0; p < 2; p++) {
                    ...
                    for (int i = 0; i < 4; i++) {
                        for (int j = 0; j < 4; j++) {
                            H_local[i][j] += alfa * el.powierzchnieLokalne[id].N[p][i] * el.powierzchnieLokalne[id].N[p][j] * detj;
                        }
                        if(id == 0){
                            P_local[i] += alfa * gd.ambientTemperature * el.powierzchnieLokalne[id].N[p][i] * detj;
                        }
                        if(id ==2){
                            P_local[i] += alfa * gd.ambientTemperature2 * el.powierzchnieLokalne[id].N[p][i] * detj;
                        }
